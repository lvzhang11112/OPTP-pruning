...
class TorchNaiveGreedy(optimizer):
    def __init__(self, index, budget: int, already_selected=[]):
        super(TorchNaiveGreedy, self).__init__(index, budget, already_selected)
    ...
    def select(self, gain_function, update_state=None, **kwargs):
        ...
        selected = torch.zeros(self.n, dtype=torch.bool)
        selected[self.already_selected] = True

        greedy_gain = torch.zeros(len(self.index))
        for i in range(selected.sum().item(), self.budget):
            greedy_gain[~selected] = gain_function(~selected, selected, **kwargs)
            current_selection = greedy_gain.argmax().item()
            selected[current_selection] = True
            greedy_gain[current_selection] = float('-inf')
            if update_state is not None:
                update_state(torch.tensor([current_selection]), selected, **kwargs)
        return self.index[selected.cpu().numpy()]