...
class SubmodularFunction(object):
    def __init__(self, index, cls_attn=None, similarity_kernel=None, similarity_matrix=None, already_selected=[]):
        self.index = index
        self.n = len(index)
        self.attns = cls_attn
...
class FacilityLocation(SubmodularFunction):
    ...
    def calc_gain(self, idx_gain, selected, **kwargs):
        ...
        gains = gains * (self.attns[idx_gain]) ** float(os.environ['ALPHA'])
        return gains
...
class GraphCut(SubmodularFunction):
    ...
    def calc_gain(self, idx_gain, selected, **kwargs):
        ...
        gain = gain * (self.attns[idx_gain]) ** float(os.environ['ALPHA'])
...
class FacilityLocationTorch(SubmodularFunction):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        
        # Convert index to tensor and move to GPU if available
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        
        if self.already_selected.__len__() == 0:
            self.cur_max = torch.zeros(self.n, dtype=torch.float32, device=self.device)
        else:
            self.cur_max = torch.max(self.similarity_kernel(
                torch.arange(self.n, device=self.device), 
                self.already_selected), dim=1)[0]

        self.all_idx = torch.ones(self.n, dtype=torch.bool, device=self.device)

    def _similarity_kernel(self, similarity_kernel):
        # Initialize tensors to store similarity values
        self.sim_matrix = torch.zeros([self.n, self.n], dtype=torch.float32, device=self.device)
        self.if_columns_calculated = torch.zeros(self.n, dtype=torch.bool, device=self.device)
        ...
        def _func(a, b):
            if not torch.all(self.if_columns_calculated[b]):
                ...
                self.sim_matrix[:, not_calculated] = similarity_kernel(self.all_idx, not_calculated)
                self.if_columns_calculated[not_calculated] = True
            return self.sim_matrix[torch.meshgrid(a, b, indexing='ij')]
        return _func

    def calc_gain(self, idx_gain, selected, **kwargs):
        gains = torch.maximum(
            torch.tensor(0., device=self.device),
            self.similarity_kernel(self.all_idx, idx_gain) - self.cur_max.reshape(-1, 1)
        ).sum(dim=0)
        
        # Convert attns to tensor if it's not already
        if not isinstance(self.attns, torch.Tensor):
            self.attns = torch.tensor(self.attns, device=self.device)
    ...
    def calc_gain_batch(self, idx_gain, selected, **kwargs):
        ...
        gains = torch.maximum(
            torch.tensor(0., device=self.device),
            self.similarity_kernel(batch_idx, idx_gain) - self.cur_max[batch_idx].reshape(-1, 1)
        ).sum(dim=0)

        for i in range(kwargs["batch"], self.n, kwargs["batch"]):
            batch_idx = ~self.all_idx
            batch_idx[i:min(i + kwargs["batch"], self.n)] = True
            gains += torch.maximum(
                torch.tensor(0., device=self.device),
                self.similarity_kernel(batch_idx, idx_gain) - self.cur_max[batch_idx].reshape(-1, 1)
            ).sum(dim=0)
    ...
    def update_state(self, new_selection, total_selected, **kwargs):
        self.cur_max = torch.maximum(
            self.cur_max,
            torch.max(self.similarity_kernel(self.all_idx, new_selection), dim=1)[0]
        )