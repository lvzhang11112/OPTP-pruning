...
class LlamaModel(LlamaPreTrainedModel):
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if rank_layer in self.layer_list:
                if hidden_states.shape[1]!=1:  # prefill stage or training
                    ...
                    stage = self.layer_list.index(rank_layer) # determine current stage
                    cur_visual_length = [int(cur_image_token * self.image_token_ratio_list[stage]) if self.image_token_ratio_list[stage] <= 1 else self.image_token_ratio_list[stage] for cur_image_token in self.image_tokens]
                    next_visual_length = [int(cur_image_token * self.image_token_ratio_list[stage + 1]) if self.image_token_ratio_list[stage + 1] <= 1 else self.image_token_ratio_list[stage + 1] for cur_image_token in self.image_tokens]
                    new_position_ids = []
    ...
    def pdrop_rank_drop(
        ...
        if getattr(self.config, 'tokenizer_padding_side', 'right') == "right":
            ...
            image_tokens = [int(cur_image_token * self.image_token_ratio_list[cur_num]) if self.image_token_ratio_list[cur_num] <= 1 else self.image_token_ratio_list[cur_num] for cur_image_token in self.image_tokens]
            keep_length = [int(cur_image_token * self.image_token_ratio_list[cur_num + 1]) if self.image_token_ratio_list[cur_num + 1] <= 1 else self.image_token_ratio_list[cur_num + 1] for cur_image_token in self.image_tokens]
